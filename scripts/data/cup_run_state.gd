...
func to_dict() -> Dictionary:
	return {"schema_version": schema_version, "run_id": str(run_id), "cup_id": str(cup_id), "difficulty_id": str(difficulty_id), "cc_id": str(cc_id), "variant_id": str(variant_id), "race_seed": race_seed, "current_race_index": current_race_index, "standings": standings.duplicate(true), "committed_races": Array(committed_races), "is_completed": is_completed}
static func from_dict(data: Dictionary) -> CupRunState:
	...
	state.variant_id = StringName(data.get("variant_id", ""))
