...
func _update_icon():
	...
	if display_icon == Icons.KEYBOARD:
		...
		if ev is InputEventKey:
			var textures = manager.get_keyboard_textures()
			texture = textures.get_texture(ev)
		elif ev is InputEventMouseButton:
			var textures = manager.get_mouse_textures()
			texture = textures.get_texture(ev)
	else:
		var types = [InputEventJoypadButton, InputEventJoypadMotion]
		var ev = _find_event(events, types)
		if ev is InputEventJoypadButton:
			var textures = manager.get_joypad_button_textures(display_icon)
			texture = textures.get_texture(ev)
		elif ev is InputEventJoypadMotion:
			var textures = manager.get_joypad_motion_textures(display_icon)
			texture = textures.get_texture(ev)
	queue_redraw()
